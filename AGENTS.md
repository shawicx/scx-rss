...
**Codex / Cursor / Copilot**: 在开始任何工作前，请先阅读 [.wiki/ai-context.md](.wiki/ai-context.md) 以最快速度理解项目核心架构。
...
Frontend-driven via `useAutoUpdate` composable → `@tauri-apps/plugin-updater` checks GitHub Releases `latest.json` (minisign signature verification). App checks automatically 3s after startup; Settings「关于」区 provides a manual「检查更新」entry. Private key is injected via CI secret (`TAURI_SIGNING_PRIVATE_KEY`); public key is embedded in `tauri.conf.json`. No OS-level code signing (Gatekeeper/SmartScreen will warn on first install). See `.wiki/backend.md`.
